...
export default function ContentPlatformView({
  userId,
  platform,
  stats,
}: {
  ...
  function handleSaved(row: ContentStat) {
  ...
  function handleImported(rows: ContentStat[]) {
    setEntries((prev) => {
      const imported = rows.filter((r) => r.platform === platform);
      const importedDates = new Set(imported.map((r) => r.stat_date));
      return [
        ...imported,
        ...prev.filter((r) => !importedDates.has(r.stat_date)),
      ];
    });
  }
    ...
    <main className="flex flex-col gap-6 p-8 text-ivory-100">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          <ContentCsvUpload userId={userId} t={t} onImported={handleImported} />
          <button
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-slate-400">
                ...
                <th className="pb-2 text-right font-normal">{t.reachLabel}</th>
                <th className="pb-2 text-right font-normal">{t.clicksLabel}</th>
                <th className="pb-2 text-right font-normal">
                  {t.engagementLabel}
                </th>
                <th className="pb-2 text-right font-normal">{t.ctrLabel}</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((row) => (
